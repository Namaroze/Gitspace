	...
	class Tmsht_Settings_Tabs extends Bws_Settings_Tabs {
		...
		public function save_options() {
			...
			if ( isset( $_POST['tmsht_add_ts_legend'] ) ) {
				...
				/*Set clear period*/
				if( ! empty( $_POST['tmsht_clear_timesheet_period'] ) ) {				
					$this->options['clear_timesheet_period'] = sanitize_text_field( $_POST['tmsht_clear_timesheet_period'] );					
					if ( $_POST['tmsht_clear_timesheet_period'] != '-' ) {
						if ( ! wp_next_scheduled( 'tmsht_clear_period_timesheet' ) ) {										
						    wp_schedule_event( time(), 'month', 'tmsht_clear_period_timesheet' );
						}
					} else {
				...
				$this->option['clear_timesheet_period'] = ( isset( $_POST['tmsht_clear_timesheet_period'] ) && in_array( ucfirst( $_POST['tmsht_clear_timesheet_period'] ), $this->period_arr ) ) ? sanitize_text_field( $_POST['tmsht_clear_timesheet_period'] ) : $this->default_options['clear_timesheet_period'];
			    ...
			    <td>
			    	...
			    	<select name="tmsht_clear_timesheet_period">
			    		<?php foreach ( $this->period_arr as $period ) { ?>
							<option value="<?php echo strtolower( $period ); ?>" <?php selected( strtolower( $period ), $this->options['clear_timesheet_period'] ); ?>> <?php _e( $period ); ?></option>
						<?php } ?>		    		
			    	</select>		     
